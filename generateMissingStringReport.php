...
<head>
    ...
    <script>
        function downloadCSV(selected_card,selected_language) {
            ...
            var win = window.open("downloadMissingStringReport.php?SelectedCard="+selected_card+"&SelectedLanguage="+selected_language, '_blank');
            win.focus();
            // alert("hello");
        }
    </script>
...
<?php
    if(isset($_POST["Submit"]) && isset($_POST["SelectedCard"]) && isset($_POST["SelectedLanguage"])) {
        ...
        $search_query = "SELECT StringKeys, English, `$column_name` FROM $tbl_name WHERE `$column_name` IS NULL OR `$column_name` = '' ";
        ...
        if($search_result = mysqli_query($connection, $search_query)) {
            if (mysqli_num_rows($search_result) > 0) {
                ...
                echo "<table border=1>";
                echo "<tr><td colspan=2>Missing Strings for Language: <b>".$column_name."</b></td></tr>";
                echo "<tr><td><b>StringKey</b></td><td><b>English</b></td></tr>";
                while($row = mysqli_fetch_assoc($search_result))
                {
                    echo "<tr>";
